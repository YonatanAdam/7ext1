...
void render_cursor(SDL_Renderer *renderer, SDL_Window *window,
  ...
  const Vec2f pos = camera_project_point(
      window, vec2f((float)editor.cursor_col * FONT_CHAR_WIDTH * FONT_SCALE,
...
int main(int argc, char **argv) {
  ...
  SDL_Window *window =
      scp(SDL_CreateWindow("7ext1", 100, 100, SCREEN_WIDTH, SCREEN_HEIGHT,
  ...
  if (!GLEW_ARB_instanced_arrays) {
    fprintf(stderr, "ARB_instanced_arrays is not supported; game may not work "
                    "properly!!\n");
    exit(1);
  }
  ...
  // Initialize Shaders
  {
    ...
    scale_unifrom = glGetUniformLocation(program, "scale");

    glUniform2f(scale_unifrom, 5.0f, 5.0f);
  }
  ...
  while (!quit) {
...

    {
      ...
      // TODO: don't update the viewport and the resolution on actual window
      // change
      glViewport(0, 0, w, h);
...
int main(int argc, char **argv) {
  ...
  SDL_Window *window = scp(SDL_CreateWindow(
      "7ext1", 100, 100, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_RESIZABLE));
  ...
  while (!quit) {
      ...
      case SDL_MOUSEBUTTONDOWN: {
        ...
        case SDL_BUTTON_LEFT: {
          Vec2f cursor_click = vec2f_add(
              mouse_click, vec2f_sub(camera_pos, vec2f_div(window_size(window),
                                                           vec2fs(2.0f))));
          if (cursor_click.x > 0.0f && cursor_click.y > 0.0f) {
            editor.cursor_col = (size_t)floorf(
                cursor_click.x / ((float)FONT_CHAR_WIDTH * FONT_SCALE));
            editor.cursor_row = (size_t)floorf(
                cursor_click.y / ((float)FONT_CHAR_HEIGHT * FONT_SCALE));
          }
        } break;
        }
      } break;
    ...
    for (size_t row = 0; row < editor.size; ++row) {
      ...
      const Vec2f line_pos = camera_project_point(
          window, vec2f(0.0f, (float)row * FONT_CHAR_HEIGHT * FONT_SCALE));
      render_text_sized(renderer, &font, line->chars, line->size, line_pos,
